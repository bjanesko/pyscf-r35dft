...
# Posted by anatoly techtonik, modified by community. See post 'Timeline' for change history
...
def eval_rho(mol, ao, dm, non0tab=None, xctype='LDA', hermi=0,
             with_lapl=True, verbose=None, r35beta=None,aor35=None):
	...
	BGJ: R35 functionals take aor35 input and return (6,N) array where last
        two rows are tau and eta35. 
    ...
    '''
    if verbose is not None and verbose >= 5:
    ...
    if r35beta is None and (xctype == 'LDA' or xctype == 'HF'): # BGJ
        ...
        if r35beta is not None: # BGJ - check R35 first
            if(with_lapl):
              raise NotImplementedError('density laplacian not supported with rung 3.5 terms')
            # rho[4] = 1/2 |nabla f|^2, rho[5] = [integral over r'] 1PDM(r,r') exp[-beta*|r-r'|^2]
        ...
        if r35beta is not None:# BGJ 
          rho[5] = _contract_rho(aor35, c0)
...
def eval_rho1(mol, ao, dm, screen_index=None, xctype='LDA', hermi=0,
    ...
    '''
    if verbose is not None and verbose >= 5:
        print('eval_rho1 called with xctype = %s' % xctype)
...
def eval_rho2(mol, ao, mo_coeff, mo_occ, non0tab=None, xctype='LDA',
    ...
    if numpy.any(pos):
        ...
        if r35beta is None and (xctype == 'LDA' or xctype == 'HF'): # BGJ
            ...
            rho[0] = _contract_rho(c0, c0)
            if r35beta is not None: # BGJ 
              c35 = _dot_ao_dm(mol, aor35, cpos, non0tab, shls_slice, ao_loc)
              rho[5] = _contract_rho(c35, c0)
        ...
        if r35beta is None and (xctype == 'LDA' or xctype == 'HF'): # BGJ
            ...
            if aor35 is not None: # BGJ - check R35 first
    ...
    if numpy.any(neg):
        ...
        if r35beta is None and (xctype == 'LDA' or xctype == 'HF'): # BGJ
            ...
            #:rho -= numpy.einsum('pi,pi->p', c0, c0)
            rho -= _contract_rho(c0, c0)
        elif r35beta is None and xctype == 'GGA': # BGJ 
            ...
            #:rho[0] -= numpy.einsum('pi,pi->p', c0, c0)
            rho[0] -= _contract_rho(c0, c0)
            if(aor35 is not None): # BGJ 
              c35 = _dot_ao_dm(mol, aor35[0], cneg, non0tab, shls_slice, ao_loc)
              rho[5] -= _contract_rho(c35, c0)
...
def nr_rks(ni, mol, grids, xc_code, dms, relativity=0, hermi=1,
           max_memory=2000, verbose=None, molr35=None,r35beta=None):  # BGJ
    ...
    if verbose is not None and verbose >= 5:
        print('+++ xctype in nr_rks: ',xctype)  # BGJ
        print('+++ making make_rho with r35beta ',r35beta)
    make_rho, nset, nao = ni._gen_rho_evaluator(mol, dms, hermi, False, grids, r35beta)
    ao_loc = mol.ao_loc_nr()
    ...
    def block_loop(ao_deriv):
        for ao, mask, weight, coords \
            ...
            # BGJ
            aor35 = None
            if(molr35 is not None):
                aor35 =  ni.eval_ao(molr35, coords, deriv=0, non0tab=mask,
                        cutoff=grids.cutoff)
                if verbose is not None and verbose >= 5:
                    print('+++ ao and aor35 shape ',ao.shape,aor35.shape)
            for i in range(nset):
                rho = make_rho(i, ao, mask, xctype, aor35)
                ngrids = rho.shape[1]
                if verbose is not None and verbose >= 5:
                    print('+++ made rho with shape ',rho.shape,' and aor35 ',aor35 is not None)
                # BGJ hack: rho or tau equal to zero causes divergences 
                rhomin=1e-12*numpy.ones_like(rho[0])
                rho[0]=numpy.maximum(rhomin,rho[0])
                if(xctype == "MGGA"):  # BGJ
                    rho[4]=numpy.maximum(rhomin,rho[4])
                exc, vxc = ni.eval_xc_eff(xc_code, rho, deriv=1, xctype=xctype,r35beta=r35beta,weight=weight)[:2]
                if r35beta is None and xctype == 'LDA': # BGJ
                    den = rho * weight
                else:
                    den = rho[0] * weight
                nelec[i] += den.sum()
                ...
                wv = weight * vxc
                yield i, ao, mask, wv, aor35 # BGJ need to pass aor35 for the potential

    aow = None
    pair_mask = mol.get_overlap_cond() < -numpy.log(ni.cutoff)
    if r35beta is None and xctype == 'LDA': # BGJ 
        ao_deriv = 0
        for i, ao, mask, wv, aor35 in block_loop(ao_deriv): 
    ...
    elif r35beta is None and xctype == 'NLC': # BGJ 
    ...
    elif r35beta is not None or xctype == 'MGGA':
        if (any(x in xc_code.upper() for x in ('CC06', 'CS', 'BR89', 'MK00'))):
            raise NotImplementedError('laplacian in meta-GGA method')
        ao_deriv = 1
        v1 = numpy.zeros_like(vmat)
        v2 = numpy.zeros_like(vmat) # BGJ array v2 will hold the R35 contribution 
        for i, ao, mask, wv, aor35 in block_loop(ao_deriv):
            ...
            # BGJ nonHermitian array v2[i,mu,nu] is ao[mu,igrid]*w[5,igrid]*aor35[nu,igrid]
            if(aor35 is not None):
              aow = _scale_ao_sparse(ao[0], wv[5], mask, ao_loc, out=aow)
              _dot_ao_ao_sparse(aor35, aow, None, nbins, mask, pair_mask, ao_loc,
                                hermi=0, out=v2[i])
              v2[i] = (v2[i] + v2[i].T)*.5 # Symmetrize v2 
        vmat = lib.hermi_sum(vmat, axes=(0,2,1))
        vmat += v1
        vmat += v2 
        # BGJ print the linear fit terms 
        if(hasattr(ni,'elhs')) and verbose is not None:
          for i in range(5):
            print('R35LIN  %1d %12.6f %12.6f '%(i,ni.elhs[i],ni.eb05s[i]))
...
def nr_uks(ni, mol, grids, xc_code, dms, relativity=0, hermi=1,
    ...
    xctype = ni._xc_type(xc_code)
    if verbose is not None and verbose >= 5:
        print(f'nr_uks called by {caller_name()}')  # BGJ
    if xctype == 'NLC':
        dms_sf = dms[0] + dms[1]
    ...
    nao = dma.shape[-1]
    make_rhoa, nset = ni._gen_rho_evaluator(mol, dma, hermi, False, grids,r35beta)[:2]
    make_rhob       = ni._gen_rho_evaluator(mol, dmb, hermi, False, grids,r35beta)[0]
    ...
    def block_loop(ao_deriv):
        for ao, mask, weight, coords \
            ...
            if(molr35 is not None):
              ...
              #print('+++ ao and aor35 shape ',ao.shape,aor35.shape)
            for i in range(nset):
                rho_a = make_rhoa(i, ao, mask, xctype, aor35)
                rho_b = make_rhob(i, ao, mask, xctype, aor35)
                rho = (rho_a, rho_b)
                ngrids = rho_a.shape[1]
                # BGJ hack: tau equal to zero causes divergences 
                rhomin=1e-12*numpy.ones_like(rho[0][0])
                for ispin in range(2) :
                  rho[ispin][0]=numpy.maximum(rhomin,rho[ispin][0])
                  rho[ispin][4]=numpy.maximum(rhomin,rho[ispin][4])
                exc, vxc = ni.eval_xc_eff(xc_code, rho, deriv=1, xctype=xctype,r35beta=r35beta,weight=weight,coords=coords)[:2]
                if r35beta is None and xctype == 'LDA': # BGJ
                    den_a = rho_a * weight
                    den_b = rho_b * weight
                else:
                ...
                wv = weight * vxc
                yield i, ao, mask, wv, aor35 # BGJ need to pass these for potential
    ...
    if r35beta is None and xctype == 'LDA': # BGJ 
        ...
        for i, ao, mask, wv, aor35 in block_loop(ao_deriv):
            ...
            # BGJ 
            if(aor35 is not None):
              aow = _scale_ao_sparse(ao[0], wva[5], mask, ao_loc, out=aow)
              _dot_ao_ao_sparse(aor35, aow, None, nbins, mask, pair_mask, ao_loc,
                                hermi=0, out=v2[0,i])
              v2[0,i] = (v2[0,i] + v2[0,i].T)*.5 # Symmetrize v2 
              aow = _scale_ao_sparse(ao[0], wvb[5], mask, ao_loc, out=aow)
              _dot_ao_ao_sparse(aor35, aow, None, nbins, mask, pair_mask, ao_loc,
                                hermi=0, out=v2[1,i])
              v2[1,i] = (v2[1,i] + v2[1,i].T)*.5 # Symmetrize v2 
        vmat = lib.hermi_sum(vmat.reshape(-1,nao,nao), axes=(0,2,1)).reshape(2,nset,nao,nao)
        vmat += v1
        vmat += v2
        # BGJ print the linear fit terms 
...
# Overwrite following functions to use custom XC functional
    ...
    def eval_xc_eff(self, xc_code, rho, deriv=1, omega=None, xctype=None,
                    verbose=None,r35beta=None,weight=None,coords=None):
        ...
        if spin_polarized:
            ...
            spin = 1
            if rhop.shape[1] == 6:  # BGJ R35  MGGA
                ngrids = rhop.shape[2]
                rhop = numpy.empty((2, 7, ngrids))
                rhop[0,:4] = rho[0][:4]
                rhop[1,:4] = rho[1][:4]
                rhop[:,4] = 0
                rhop[0,5] = rho[0][4]
            ...
            spin = 0
            if rhop.shape[0] == 6:  # BGJ R35 MGGA
                ngrids = rho.shape[1]
                rhop = numpy.empty((7, ngrids))
                rhop[:4] = rho[:4]
                rhop[4] = 0
                rhop[5] = rho[4]
                rhop[6] = rho[5]
        ...
        short_length = 4 if xctype == 'GGA' else (5 if xctype == 'MGGA' else 6) # LAST ELSE HERE MIGHT BE WRONG. 
        if xctype == 'MGGA':
            # For MGGA, we need to properly construct short_rho with τ at index 4
            # After the transformation above, rhop has layout [ρ, ∇x, ∇y, ∇z, 0, τ, (η35)]
            # We need to create [ρ, ∇x, ∇y, ∇z, τ] for the base MGGA functional
            if spin == 1:
                ngrids = rhop.shape[2]
                short_rho = numpy.empty((2, 5, ngrids))
                short_rho[:, :4, :] = rhop[:, :4, :]  # ρ, ∇x, ∇y, ∇z
                short_rho[:, 4, :] = rhop[:, 5, :]    # τ (skip index 4 which is 0)
            else:
                ngrids = rhop.shape[1]
                short_rho = numpy.empty((5, ngrids))
                short_rho[:4, :] = rhop[:4, :]  # ρ, ∇x, ∇y, ∇z
                short_rho[4, :] = rhop[5, :]    # τ (skip index 4 which is 0)
        elif spin == 1:
            short_rho = rhop[:,:short_length]
        else:
            short_rho = rhop[:short_length]
        exc, vxc, fxc, kxc = self.eval_xc(xc_code, short_rho, spin, 0, deriv, omega,
                                          verbose)
        if deriv > 2:
        ...
        # BGJ R35 correction. The output tensor is expanded to the shape 
        #    LDA : [1,N]
        #    GGA : [2-4,N]
        #    MGGA: [5,N]
        # R35MGGA: [6,N]
        if r35beta is not None:
          ...
          if spin_polarized:
            ...
            rholen = rhop.shape[0]
          if rholen < 5: 
            raise NotImplementedError('R35 are implented as MGGA and your rhop shape is ',rhop.shape[0])
          if deriv > 1:
            raise NotImplementedError('Derivatives >1 not supported with rung 3.5 terms')
          if not spin_polarized:
             nslot,ngrid = vxc.shape
             vxc2 = numpy.zeros((6,ngrid))
             vxc2[:nslot]=vxc[:nslot]
          else:      
             nspin,nslot,ngrid = vxc.shape
             vxc2 = numpy.zeros((nspin,6,ngrid))
             vxc2[:,:nslot,:]=vxc[:,:nslot,:]
          exc, vxc = r35work.r35c(self,rhop,spin,exc,vxc2,r35beta,weight,coords,verbose) 
          #if not spin_polarized:
          #   print('+++ VEND',vxc[:,1])
          #else:      
          #   print('+++ VEND',vxc[:,:,1])
...
class NumInt(_NumIntMixin):
    ...
    def _gen_rho_evaluator(self, mol, dms, hermi=0, with_lapl=True, grids=None, r35beta=None):
        if getattr(dms, 'mo_coeff', None) is not None:
        ...
        def make_rho(idm, ao, sindex, xctype, aor35=None): # BGJ 
            if sindex is not None and grids is not None and aor35 is None :
                ...
                return self.eval_rho2(mol, ao, mo_coeff[idm], mo_occ[idm],
                                      sindex, xctype, with_lapl, r35beta=r35beta, aor35=aor35)
            else:
                return self.eval_rho(mol, ao, dms[idm], sindex, xctype, hermi,
                                     with_lapl, r35beta=r35beta,aor35=aor35 )

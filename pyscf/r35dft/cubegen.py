...
# BGJ 
def interp(typ, mol, outfile, dm, nx=80, ny=80, nz=80, resolution=RESOLUTION,
            margin=BOX_MARGIN):
    """Calculates various interpretive tools and write out in cube format.
       See function 'density' for arguments 
       Current values of first argument 'typ': 
       ELF:  electron localization function
       EDRXXXX: electron delocalization range function with length scale d set to XXXX/1000 au
    """
    if(len(dm.shape)<3):
        sys.exit('cubegen interp currently requires UHF density matrices ')
    ...
    molr35=None 
    if('ELF' in typ.upper()):
        comment='Electron localization function '
    elif('EDR' in typ.upper()):
        d=float(typ.upper().replace('EDR',''))/1000.
        comment='Electron delocalization range function, distance d= %.2f au'%(d)
        beta=d**(-2)
        molr35 = r35dft.r35work.stretchAOs(mol,beta)
    else:
        sys.exit('Bad input ',typ,' in cubegen interp')

    if mol.cart:
      feval='GTOval_cart_deriv1'
      reval='GTOval_cart'
    else:
      feval='GTOval_sph_deriv1'
      reval='GTOval_sph'

    # Compute interpretive tool on the .cube grid
    coords = cc.get_coords()
    ngrids = cc.get_ngrids()
    blksize = min(8000, ngrids)
    val = numpy.empty(ngrids)
    for ip0, ip1 in lib.prange(0, ngrids, blksize):
        ao = mol.eval_gto(feval, coords[ip0:ip1])
        if(beta<0):
          rhoa = r35dft.numint.eval_rho(mol,ao,dm[0],xctype='MGGA')
          rhob=r35dft.numint.eval_rho(mol,ao,dm[1],xctype='MGGA') 
          rho = numpy.array((rhoa,rhob))
          omelfa=r35dft.r35work.get_omELF(rho,l=1)[0][0]
          val[ip0:ip1] = -1.0*omelfa + 1.0*numpy.ones_like(omelfa)
        else:
          aor35 = molr35.eval_gto(reval, coords[ip0:ip1])
          rhoa = r35dft.numint.eval_rho(mol,ao,dm[0],xctype='MGGA',r35beta=[beta],aor35=aor35,with_lapl=False)
          r = rhoa[0]
          r = numpy.maximum(r,1e-10*numpy.ones_like(r))
          edr=(rhoa[5]**2)**0.5/r**0.5
          val[ip0:ip1] = edr

    val = val.reshape(cc.nx,cc.ny,cc.nz)

    # Write out density to the .cube file
    cc.write(val, outfile, comment=comment)
    return val 
...
def density(mol, outfile, dm, nx=80, ny=80, nz=80, resolution=RESOLUTION,
    ...
    for ip0, ip1 in lib.prange(0, ngrids, blksize):
        ...
        rho[ip0:ip1] = r35dft.numint.eval_rho(mol, ao, dm)
